...
class LeonardoMouseDriver:
    ...
    def _connect(self):
        try:
            ...
            if self.port == "auto":
                ...
                if detected is None:
                    log("未检测到 Leonardo，请检查 USB 连接", "ERROR")
                    return
                self.port = detected
                log(f"自动检测到 Leonardo: {self.port}", "SUCCESS")
            self.ser = serial.Serial(port=self.port, baudrate=self.baud, timeout=0.05, write_timeout=0.05)
            time.sleep(2.0)
            if self._closed:
                try:
                    self.ser.close()
    ...
    def move(self, dx, dy) -> bool:
        if math.isnan(dx) or math.isnan(dy):
            return False
        dx = max(-127, min(127, int(round(dx))))
        dy = max(-127, min(127, int(round(dy))))
        if dx == 0 and dy == 0:
            return False
    ...
    def click_left(self) -> bool:
        return self._send_cmd(self.CMD_CLICK)
